...
fn main() {
    ...
    if args.len() != 5 {
        eprintln!(
            "Usage: {} <kv_count> <band_width> <columns> <total_runs>",
            args[0]
        );
        ...
        .for_each(|(_chunk_idx, &start_run)| {
            ...
            (start_run..end_run).into_par_iter().for_each(|run| {
                ...
                for i in 0..kv_count {
                    ...
                    let key: Vec<bool> = (0..key_length)
                        .map(|_| local_rng.random_bool(0.5))
                        .collect();
    ...
    if final_failures > 0 {
        ...
        println!(
            "Failure rate: {:.6}% ({}/{}) | log2(runs/failures): {:.2}",
        ...
        println!(
            "Failure rate: 0.000000% (0/{}) | log2(runs/failures): ∞ (no failures)",
            total_runs
        );
    }
    println!();
    ...
    if final_successes > 0 {
        println!(
            "Average encoding time (successful): {:?}",
            total_encode_time_final / final_successes as u32
        );
    }
    println!(
        "Throughput: {:.2} runs/second",
        total_runs as f64 / total_time.as_secs_f64()
    );
    println!(
        "Parallelization speedup: ~{}x (estimated)",
        rayon::current_num_threads()
    );
    println!();
    ...
    if final_failures == 0 {
        ...
        println!(
            "   Failure probability: {:.2e}",
            final_failures as f64 / total_runs as f64
        );
        println!(
            "   Expected failures per million runs: {:.0}",
        ...
        let confidence_interval = 1.96
            * ((final_failures as f64 / total_runs as f64)
                * (1.0 - final_failures as f64 / total_runs as f64)
                ...
                .sqrt();
        println!(
            "95% confidence interval for failure rate: ±{:.2e}",
            confidence_interval
        );
    }
    println!(
        "Sample size: {} runs provides good statistical power",
        total_runs
    );
    println!(
