...
impl<const N: usize> IntervalFSSCW<N> {
    pub fn bytes_size(modulus: u128) -> usize {
        // Calculate the size of the serialized IntervalFSSCW<N> structure
        let mut size = 0;
...
impl<const N: usize> IntervalFSSKey<N> {
    ...
    pub fn from_bytes(bytes: &[u8], modulus: u128) -> (Self, usize) {
        ...
        for _ in 0..num_words {
            let (cw, used) = IntervalFSSCW::<N>::from_bytes(&bytes[offset..], modulus);
            cor_words.push(cw);
            offset += used;
        }
        (
...
impl<const N: usize> IntervalFSSKey<N>
{
    ...
    // Need alpha < beta
    pub fn gen_interval_fss_key(alpha_bits: &[bool], beta_bits: &[bool], a: &RingVec<N>, b: &RingVec<N>, c: &RingVec<N>, modulus: u128) -> (IntervalFSSKey<N>, IntervalFSSKey<N>) {
    ...
    pub fn eval_interval_fss(&self, idx: &[bool], modulus: u128) -> RingVec<N> {
