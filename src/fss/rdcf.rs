...
impl RdcfCW {
    fn check_wellformedness(&self) -> Result<()> {
            ...
            "Size mismatch between ys of RdcfCW, got {} and {}",
            self.ys.0.len(),
            self.ys.1.len()
        );
        ensure!(
            self.ys.0.modulus() == self.ys.1.modulus(),
            "Modulus mismatch between ys of RdcfCW, got {} and {}",
            self.ys.0.modulus(),
            self.ys.1.modulus()
        );
        Ok(())
    ...
    pub fn bytes_size(&self) -> Result<usize> {
        self.check_wellformedness()
            .map_err(|err| anyhow!("Wrong form for RdcfCW: {}", err))?;
        let mut size = 0;
        size += AES_BLOCK_SIZE;
        size += 1; // for seed_bit
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        let bytes_size = self
            .bytes_size()
            .map_err(|err| anyhow!("Cannot calculate RdcfCW bytes size: {}", err))?;
    ...
    pub fn from_bytes(bytes: &[u8], modulus: u128) -> Result<(Self, usize)> {
            ...
            "Not enough bytes to read RdcfCW header"
        );
        let mut offset = 0;
...
impl RdcfKey {
    ...
    pub fn from_bytes(bytes: &[u8], modulus: u128) -> Result<(Self, usize)> {
            ...
            "Not enough bytes to read RdcfKey header"
        );
        let mut offset = 0;
        ...
        for _ in 0..num_words {
            let (cw, used) = RdcfCW::from_bytes(&bytes[offset..], modulus)
                .context("Invalid RdcfCW from bytes")?;
            cor_words.push(cw);
            offset += used;
        }
        Ok((
...
impl RdcfEval {
    ...
    pub fn from_bytes(bytes: &[u8], modulus: u128) -> Result<(Self, usize)> {
            ...
            "Not enough bytes to read RdcfEval header"
        );
        ensure!(
            modulus.is_power_of_two(),
...
fn gen_layer_data(key: [u8; AES_BLOCK_SIZE], modulus: u128, payload_len: usize) -> Result<RdcfData> {
    ensure!(
        modulus.is_power_of_two(),
        "Modulus must be a power of 2 and greater than 0"
    );
...
fn gen_cor_word(
    ...
    if !alpha_bit {
        ...
        cw.ys = (delta_ys.0 + left.clone(), delta_ys.1 + left.clone());
        cw.y_bits = (
...
impl RdcfKey {
    pub fn gen_rdcf_key(
        ...
        let mut cor_words: Vec<RdcfCW> = Vec::with_capacity(u);
    ...
    pub fn eval_bit(&self, state: &RdcfEval, modulus: u128, dir: bool) -> Result<RdcfEval> {
        ensure!(
        ...
        let data: RdcfData = gen_layer_data(state.seed, modulus, payload_len)?;
    ...
    pub fn expand_prefix(&self, state: &RdcfEval, modulus: u128) -> Result<(RdcfEval, RdcfEval)> {
        ensure!(
            state.level < self.cor_words.len(),
            "Eval state level {} out of bounds for {} correlation words",
            state.level,
    ...
    pub fn init_eval(&self, modulus: u128) -> Result<RdcfEval> {
        ensure!(
            modulus.is_power_of_two(),
            "Modulus must be a power of 2 and non-zero"
        );
    ...
    pub fn eval_rdcf(&self, prefix: &[bool], modulus: u128) -> Result<RingVec> {
        ensure!(
    ...
    fn validate_cw(&self, cw: &RdcfCW, payload_len: usize, modulus: u128) -> Result<()> {
        ensure!(
    ...
    fn payload_len(&self) -> Result<usize> {
        let len = self
            ...
            .first()
            .map(|cw| cw.ys.0.len())
            .ok_or_else(|| anyhow!("RdcfKey has no correlation words to determine payload length"))?;
        ensure!(
            self.cor_words
                .iter()
