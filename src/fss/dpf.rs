...
pub struct DpfKey<const N: usize> {
    pub key_idx: bool,
    pub root_seed: [u8; AES_BLOCK_SIZE],
    pub cor_words: Vec<DpfCW<N>>,
}

impl<const N: usize> DpfKey<N> {
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut out = Vec::new();
...
fn gen_layer_data<const N: usize>(key: [u8; AES_BLOCK_SIZE], modulus: u128, left: bool, right: bool) -> DpfData<N> {
...
impl<const N: usize> DpfKey<N>
{
    ...
    // Need alpha < beta
    pub fn gen_DpfKey(alpha_bits: &[bool], a: &RingVec<N>, b: &RingVec<N>, modulus: u128) -> (DpfKey<N>, DpfKey<N>) {
        ...
        let mut cor_words: Vec<DpfCW<N>> = Vec::new();
