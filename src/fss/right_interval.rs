...
fn gen_cor_word<const N: usize>(
    ...
    if data.len() == 1 {
        ...
        if !alpha_bit && beta_bit {
            ...
            cw.ys.0 = (
                delta_ys[0].0.clone() + left.clone(),
                delta_ys[0].1.clone() + right0.clone(),
            );
        ...
        if !beta_bit {
            ...
            cw.ys.1 = (
                ...
                delta_ys[1].1.clone() + right1.clone(),
            );
    ...
    let new_eval: Vec<(RIntervalFSSEval<N>, RIntervalFSSEval<N>)> = new_seeds
        .iter()
        .zip(new_bits.iter())
        .zip(new_y_bits.iter())
...
impl<const N: usize> RIntervalFSSKey<N> {
    ...
    pub fn gen_rinterval_fss_key(
        ...
        c: RingVec,
        modulus: u128,
    ) -> (RIntervalFSSKey<N>, RIntervalFSSKey<N>) {
