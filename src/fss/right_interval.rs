...
fn gen_layer_data<const N: usize>(key: [u8; AES_BLOCK_SIZE], modulus: u128, left: bool, right: bool) -> RIntervalFSSData<N> {
...
) -> RIntervalFSSCW<N>
{
    ...
    if data.len() == 1 {
        ...
        if !alpha_bit && beta_bit {
            ...
            cw.ys.0 = (delta_ys[0].0 + left, delta_ys[0].1 + mid0);
        ...
        if !beta_bit {
            ...
            cw.ys.1 = (delta_ys[1].0 + mid1, delta_ys[1].1 + right1);
            ...
            cw.ys.1 = (delta_ys[1].0 + mid1, delta_ys[1].1 + mid1);
...
impl<const N: usize> RIntervalFSSKey<N>
{
    ...
    // Need alpha < beta
    pub fn gen_RIntervalFSSKey(alpha_bits: &[bool], beta_bits: &[bool], a: RingVec<N>, b: RingVec<N>, c: RingVec<N>, modulus: u128) -> (RIntervalFSSKey<N>, RIntervalFSSKey<N>) {
        ...
        let mut payload_mid1 = vec![RingVec::<N>::zero(modulus); u];
    ...
    pub fn eval_rintervalFSS(&self, idx: &[bool], modulus: u128) -> RingVec<N> {
