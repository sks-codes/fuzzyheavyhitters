    ...
    T: crate::Share + std::cmp::PartialEq + std::fmt::Debug + From<u32>,
{
    pub fn new(
        ...
        mac_key: &T,
        mac_key2: &T,
        sketch: &sketch_dcf::SketchOutput<T>,
        level: usize,
    ) -> MulState<T> {
        debug_assert!((level + 1) * sketch_dcf::TRIPLES_PER_LEVEL <= triples.len());

        let trip_start = level * sketch_dcf::TRIPLES_PER_LEVEL;
        let trip_end = trip_start + sketch_dcf::TRIPLES_PER_LEVEL;

        let mut out = MulState {
            server_idx,
            triples: triples[trip_start..trip_end].to_vec(),

            xs: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),
            ys: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),
            zs: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),

            rs: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),
        };
    ...
    pub fn cor_share(&self) -> CorShare<T> {
        let mut out = CorShare {
            ds: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),
            es: Vec::with_capacity(sketch_dcf::TRIPLES_PER_LEVEL),
        };

        for i in 0..sketch_dcf::TRIPLES_PER_LEVEL {
    ...
    T: crate::Share + std::cmp::PartialEq + std::fmt::Debug + From<u32>,
{
    ...
    pub fn new(
        ...
        mac_keys: &[T],
        mac_keys2: &[T],
        sketch: &Vec<sketch_dcf::SketchOutput<T>>,
        level: usize,
    ) -> ManyMulState<T> {
    ...
    pub fn verify(out0: &ManyOutShare<T>, out1: &ManyOutShare<T>) -> Vec<bool> {
        ...
        for i in 0..out0.out_shares.len() {
            let smth = MulState::verify(&out0.out_shares[i], &out1.out_shares[i]);
            print!("{:?}",smth);
            out.push(smth);
