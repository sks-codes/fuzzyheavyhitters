...
pub fn geo_to_grid(lat: f64, lon: f64) -> (u32, u32) {
    ...
    (
        if lat_grid < LAT_GRID_SIZE {
            lat_grid
        } else {
            LAT_GRID_SIZE - 1
        },
        if lon_grid < LON_GRID_SIZE {
            lon_grid
        } else {
            LON_GRID_SIZE - 1
        },
...
pub fn csv_to_bitvecs<P: AsRef<Path>>(path: P) -> Result<Vec<Vec<Vec<bool>>>, Box<dyn Error>> {
