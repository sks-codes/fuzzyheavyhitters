...
impl FuzzyHeavyHittersProtocol {
    ...
    fn batch_test_prefix_sets_threshold_parallel(
        ...
        server_bits
            ...
            .enumerate()
            .try_for_each(|(chunk_idx, (result_chunk, prefix_chunk))| {
                ...
                let dealer_channel = if chunk_idx < dealer_channels.len() {
                    dealer_channels[chunk_idx].clone()
                } else {
                    // Fallback to round-robin if more chunks than channels
                    dealer_channels[chunk_idx % dealer_channels.len()].clone()
                };
                ...
                for (local_idx, prefix_set) in prefix_chunk.iter().enumerate() {
                    ...
                    let check_data_list = if matches!(self.config.check_config.method, CheckMethod::LpIntervalFSS) {
                        ...
                        let mut locked_dealer_channel = dealer_channel.lock().map_err(|e| format!("Failed to lock dealer channel: {}", e))?;
                        let batch = self.request_dealer(DealerSignal::RequestCheckKeys, &mut *locked_dealer_channel, 1u128 << self.config.check_config.output_bit_length)?;
                        drop(locked_dealer_channel);
                        ...
                        let single_check_data = match self.config.check_config.method {
                            ...
                            CheckMethod::LpIntervalFSS => CheckData::LpGarbledCircuits { threshold: distance_threshold }, // Fallback
                        };
                    ...
                    let threshold_data_to_use = if matches!(self.config.threshold_config.method, ThresholdMethod::IntervalFSS) {
                        ...
                        let mut locked_dealer_channel = dealer_channel.lock().map_err(|e| format!("Failed to lock dealer channel: {}", e))?;
                        let batch = self.request_dealer(DealerSignal::RequestThresholdKeys, &mut *locked_dealer_channel, 2u128)?;
                        drop(locked_dealer_channel);
