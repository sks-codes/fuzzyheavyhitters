...
pub fn generate_interval_fss_sketch_data(
    ...
    for _ in 0..num_clients {
        ...
        let b = (0..2*d).map(|_| rand::random::<u128>() % sketch_modulus).collect::<Vec<u128>>();
        ...
        let a3_0 = (0..2*d).map(|_| rand::random::<u128>() % sketch_modulus).collect::<Vec<u128>>();
        let a4_0 = (0..2*d).map(|_| rand::random::<u128>() % sketch_modulus).collect::<Vec<u128>>();
        let b_0 = (0..2*d).map(|_| rand::random::<u128>() % sketch_modulus).collect::<Vec<u128>>();

        let a_1 = a.iter().zip(a_0.iter()).map(|(&x, &y)| (sketch_modulus + x - y) % sketch_modulus).collect::<Vec<u128>>();
        let a2_1 = a2.iter().zip(a2_0.iter()).map(|(&x, &y)| (sketch_modulus + x - y) % sketch_modulus).collect::<Vec<u128>>();
        let a3_1 = a3.iter().zip(a3_0.iter()).map(|(&x, &y)| (sketch_modulus + x - y) % sketch_modulus).collect::<Vec<u128>>();
        let a4_1 = a4.iter().zip(a4_0.iter()).map(|(&x, &y)| (sketch_modulus + x - y) % sketch_modulus).collect::<Vec<u128>>();
        let b_1 = b.iter().zip(b_0.iter()).map(|(&x, &y)| (sketch_modulus + x - y) % sketch_modulus).collect::<Vec<u128>>();

        sketch_data_0.push(SketchData::IntervalFSS { a: a_0, a2: a2_0, a3: a3_0, a4: a4_0, b: b_0 });
        sketch_data_1.push(SketchData::IntervalFSS { a: a_1, a2: a2_1, a3: a3_1, a4: a4_1, b: b_1 });
    }
