...
use crate::garbled_circuits::batch_equality_full::{
    batch_gb_equality_test, batch_ev_equality_test};
...
impl CheckPhase {
    ...
    pub fn run_batch_fuzzy_match_check(
        ...
        match (&self.config.method, first_check_data) {
            (CheckMethod::Linf, CheckData::Linf) => {
                self.run_batch_linf_check(shared_ranges, query_point, channel, rng)
            }
            (CheckMethod::LpGarbledCircuits, CheckData::LpGarbledCircuits { threshold }) => {
                let threshold_modint = ModInt::new(*threshold, 1 << self.config.input_bit_length);
            ...
            (CheckMethod::LpIntervalFSS, CheckData::LpIntervalFSS { .. }) => {
                ...
                for check_data in check_data_list {
                    if let CheckData::LpIntervalFSS { threshold, fss_key, random_value } = check_data {
                ...
                // Verify all thresholds are the same for consistency
                if !thresholds.iter().all(|&t| t == thresholds[0]) {
                    return Err(CheckPhaseError::InvalidConfig(
                        "All FSS thresholds must be the same for batch processing".to_string()
                    ));
                }
    ...
    fn run_batch_lp_distance_check_intervalfss(
        ...
        for (reconstructed_distance, fss_key) in reconstructed_masked_distances.iter().zip(fss_keys.iter()) {
            let mut distance_bits = u128_to_bits(reconstructed_distance.val(), self.config.input_bit_length);
            distance_bits.reverse();
            
            let fss_result = fss_key.eval_intervalFSS(&distance_bits, out_modulus);
