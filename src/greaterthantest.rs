...
//     pub garbler_wires: Vec<BinaryBundle<F>>,  // Multiple inputs from garbler
//     pub evaluator_wires: Vec<BinaryBundle<F>>, // Multiple inputs from evaluator
// }
//
//
// /// Convert a Vec<bool> into a u128 (assuming little-endian bit order)
// fn bool_vec_to_u128(bits: &[bool]) -> u128 {
//     bits.iter()
//         .rev()
...
//
//
// /// Generalized greater-than comparison for arbitrary bit lengths
...
// {
//     let mut s = inputs.clone().to_vec();
//     // let a = s.iter().map(|i| bool_vec_to_u128(i.as_slice())).collect::<Vec<u128>>();
//     // println!("Garbler uint share: {:?}", a);
//     let mut gb = Garbler::<C, AesRng, OtSender, AllWire>::new(channel.clone(), rng.clone()).unwrap();
//     let circuit_wires = gb_set_multi_inputs(&mut gb, inputs);
...
//
//
// /// Core comparison logic for multiple inputs
...
//     {
//         // Compute a > b by checking if b - a underflows
//         // let (_, underflow) = f.bin_subtraction(&garbler_wires, &evaluator_wires)?;
//         // let res = f.negate(&underflow)?;
...
//     E: Debug,
// {
//     let garbler_wires = inputs.iter()
//         .map(|input| {
//             let val = bool_vec_to_u128(input);
//             gb.bin_encode(val, input.len()).unwrap()
//         })
//         .collect();
//     let evaluator_wires = inputs.iter()
//         .map(|input| gb.bin_receive(input.len()).unwrap())
//         .collect();
...
// {
//     let garbler_wires = inputs.iter()
//         .map(|input| ev.bin_receive(input.len()).unwrap())
//         .collect();
//
//
//     let evaluator_wires = inputs.iter()
//         .map(|input| {
//             let val = bool_vec_to_u128(input);
//             ev.bin_encode(val, input.len()).unwrap()
//         })
//         .collect();
//
//
...
//
// #[test]
// fn test_multi_greater_than() {
//     // let gb_values = vec![
//     //     vec![false, true, false, true], // 5
//     //     vec![false, true, false, true, false], // 10
//     //     vec![false, true, true, true, true], // 15
//     // ];
//     // let ev_values = vec![
//     //     vec![false, false, true, true], // 3
//     //     vec![false, true, true, false, false], // 12
//     //     vec![false, true, true, true, true], // 15
//     // ];
//     let gb_values = vec![
//         vec![true, false, true, false], // 5
//         vec![false, true, false, true, false], // 10
//         vec![true, true, true, true, false], // 15
//     ];
//     let ev_values = vec![
//         vec![true, true, false, false], // 3
//         vec![false, false, true, true, false], // 12
//         vec![true, true, true, true, false], // 15
//     ];
//
...
//         .zip(ev_values.iter())
//         .map(|(g, e)| {
//             println!("Garb: {:?}", bool_vec_to_u128(g));
//             println!("Eval: {:?}", bool_vec_to_u128(e));
//             bool_vec_to_u128(g) >= bool_vec_to_u128(e)
//         })
//         .collect();
//
...
//         multiple_gb_greater_than(&mut rng_gb.clone(), &mut channel, &gb_values);
//     });
//
...
//     let mut channel = Channel::new(reader, writer);
//
//     let results = multiple_ev_greater_than(&mut rng_ev.clone(), &mut channel, &ev_values);
