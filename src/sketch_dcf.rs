...
pub struct SketchDCFKey<T, U> {
    pub mac_key: T,
    pub mac_key2: T,
    pub mac_key_last: U,
    pub mac_key2_last: U,
    key: ibDCF::ibDCFKey<(T, T), (U, U)>,
...
impl<T,U> SketchDCFKey<T,U>
where
    T: crate::Share + std::fmt::Debug + std::cmp::PartialEq,
    U: crate::Share + std::fmt::Debug + std::cmp::PartialEq,
{
    #[allow(clippy::needless_range_loop)]
    pub fn gen(alpha_bits: &[bool], side : bool, values_in: &[T], value_last: &U) -> [SketchDCFKey<T,U>; 2] {
        ...
        let (dcf_key0, dcf_key1) = ibDCF::ibDCFKey::gen(alpha_bits, side, &values, &value_last_with_mac);
    ...
    pub fn gen_from_str(s: &str, side : bool) -> [SketchDCFKey<T,U>; 2] {
        let bits = crate::string_to_bits(s);
        let values = vec![T::one(); bits.len()-1];
        SketchDCFKey::gen(&bits, side, &values, &U::one())
    }
    ...
    pub fn sketch_at(
        ...
        let mut x_prev = T::zero();
        let mut kx_prev = T::zero();

        // if vector_in.len() > 1 {
        //     x_prev = vector_in[0].0.clone();
        //     kx_prev = vector_in[0].1.clone();
        // }

        for v in vector_in {
            if v.0 == vector_in[0].0 && v.1 == vector_in[0].1 {
                x_prev = v.0.clone();
                kx_prev = v.1.clone();
                continue;
            }
            ...
            x_diff.sub(&x_prev.clone());

            let mut kx_diff = kx.clone();
            kx_diff.sub(&kx_prev.clone());

            let mut tmp0 = x_diff.clone(); //TODO: change back to the difference later
            tmp0.mul_lazy(&sketch_r);

            let mut tmp1 = x_diff.clone(); //TODO: change back to the difference later
            tmp1.mul_lazy(&sketch_r2);

            let mut tmp2 = kx_diff.clone(); //TODO: change back to the difference later
    ...
    pub fn sketch_at_last(
        ...
        let mut x_prev = U::zero();
        let mut kx_prev = U::zero();
    ...
    pub fn eval(&self, idx: &[bool]) -> U {
        ...
        (self.key.eval(idx).1).1 //TODO: this might be strange
    }

    pub fn eval_bit(&self, state: &ibDCF::DCFEvalState, dir: bool) -> (ibDCF::DCFEvalState, T, T) {
        let (st, val) = self.key.eval_bit(state, dir);
        (st, val.0, val.1)
    }

    pub fn eval_bit_last(&self, state: &ibDCF::DCFEvalState, dir: bool) -> (ibDCF::DCFEvalState, U, U) {
        let (st, val) = self.key.eval_bit_last(state, dir);
        (st, val.0, val.1)
    }

    pub fn eval_init(&self) -> ibDCF::DCFEvalState {
        self.key.eval_init()
    }
}
...
mod tests {
    ...
    fn mac_keys() {
        ...
        let keys = SketchDCFKey::gen(&alpha, true, &betas, &beta_last);
