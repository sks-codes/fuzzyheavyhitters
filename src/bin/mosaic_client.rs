...
fn run_client(config_path: &str) -> Result<(), String> {
    ...
    let mut channel_server0 = connect_to(cli_config.network.server0_addr, cli_config.network.client_to_server0_port)
        .map_err(|e| format!("Failed to connect to server 0: {}", e))?;
    let mut channel_server1 = connect_to(cli_config.network.server1_addr, cli_config.network.client_to_server1_port)
