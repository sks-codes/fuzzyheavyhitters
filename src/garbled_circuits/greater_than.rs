    ...
    F: FancyReveal + Fancy + BinaryGadgets + FancyBinary + FancyArithmetic,
{
    ...
    for (garbler_wires, evaluator_wires) in wire_inputs
        .garbler_wires
        .into_iter()
    ...
    E: Debug,
{
    ...
    let values: Vec<Vec<u128>> = inputs
        .iter()
        .map(|input| {
    ...
    let garbler_wires = gb
        .bin_encode_many(&flat_values, nbits / values_per_input)
        .unwrap();

    let garbler_wires = garbler_wires
        .chunks(values_per_input)
        .map(|chunk| {
            if chunk.len() == 1 {
                chunk[0].clone()
            } else {
    ...
    E: Debug,
{
    ...
    let evaluator_wires = evaluator_wires
        .chunks(values_per_input)
        .map(|chunk| {
            if chunk.len() == 1 {
                chunk[0].clone()
