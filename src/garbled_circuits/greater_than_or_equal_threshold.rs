...
fn garbler_preprocess_greater_than_ss(
    ys: &[Mod2k],
    t: &Mod2k,
) -> (Vec<Vec<bool>>, Vec<Vec<bool>>, Vec<bool>) {
...
{
    let (z1_values, z2_values, b_values) = garbler_preprocess_greater_than_ss(inputs_y, input_t);
    let mut gb =
    ...
    let results = (0..inputs_y.len())
        ...
        .collect::<Vec<bool>>();
    let circuit_wires =
        gb_set_greater_than_ss_inputs(&mut gb, &z1_values, &z2_values, &b_values, &results);
    ...
    E: Debug,
{
    assert_eq!(
        z1_values.len(),
        z2_values.len(),
        "z1 and z2 must have same length"
    );
    assert_eq!(
        z1_values.len(),
        b_values.len(),
        "z1 and b must have same length"
    );
    ...
    C: AbstractChannel + Clone,
{
    ...
    let circuit_wires = ev_set_greater_than_ss_inputs(&mut ev, &z3_values);
    ...
    E: Debug,
{
    ...
    let garbler_wires = BinaryBundle::new(
        ev.receive_many(&vec![2; 2 * item_count * item_length + 2 * item_count])
            .unwrap(),
    );
    ...
    F: FancyReveal + Fancy + BinaryGadgets + FancyBinary + FancyArithmetic,
{
    ...
    for i in 0..item_count {
        let (_, carry1) = f.bin_addition(
            &BinaryBundle::new(
                garbler_wires.wires()
                    [(item_count + i) * item_length..(item_count + i + 1) * item_length]
                    .to_vec(),
            ),
            &BinaryBundle::new(
                evaluator_wires.wires()[i * item_length..(i + 1) * item_length].to_vec(),
            ),
        )?;
        let x_plus_y_overflow = carry1;
        let x_plus_y_not_overflow = f.negate(&x_plus_y_overflow)?;

        let z3_flipped = evaluator_wires.wires()[i * item_length..(i + 1) * item_length]
            .iter()
            .map(|x| f.negate(x))
        ...
        let (_, carry2) = f.bin_addition(
            &BinaryBundle::new(
                garbler_wires.wires()[i * item_length..(i + 1) * item_length].to_vec(),
            ),
            &z3_flipped,
        )?;
        ...
        let b_and_overflow2 = f.or(
            &garbler_wires[2 * item_count * item_length + i],
            &x_gt_t_minus_y,
        )?;
        let first_part = f.and(&x_plus_y_not_overflow, &b_and_overflow2)?;

        let overflow2_or_b = f.and(
            &x_gt_t_minus_y,
            &garbler_wires[2 * item_count * item_length + i],
        )?;
        ...
        let final_result = f.xor(
            &final_result,
            &garbler_wires[2 * item_count * item_length + item_count + i],
        )?;
