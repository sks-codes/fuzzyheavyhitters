...
impl DynRingVec {
    ...
    pub fn zero(len: usize, modulus: u128) -> Self {
        ...
        Self { bits: bits as u8, modulus, mask: modulus - 1, storage }
    }
    ...
    pub fn from_slice(values: &[u128], modulus: u128) -> Self {
        ...
        for (i, &val) in values.iter().enumerate() { v.set(i, val); }
        v
    }
    ...
    fn modulus_to_bits(modulus: u128) -> usize {
        assert!(modulus > 0 && (modulus & (modulus - 1)) == 0, "modulus must be power of two");
        if modulus == 1 { 1 } else { (128 - (modulus - 1).leading_zeros()) as usize }
    }

    pub fn bits(&self) -> usize { self.bits as usize }
    pub fn len(&self) -> usize { match &self.storage { RingStorage::U8(v) => v.len(), RingStorage::U16(v) => v.len(), RingStorage::U32(v) => v.len(), RingStorage::U64(v) => v.len(), RingStorage::U128(v) => v.len() } }
    pub fn modulus(&self) -> u128 { self.modulus }
    ...
    fn bin_op(&self, other: &Self, op: BinOp) -> Self {
        ...
        let storage = match (&self.storage, &other.storage) {
            (RingStorage::U8(a), RingStorage::U8(b)) => {
                ...
                for (o, (&x, &y)) in out.iter_mut().zip(a.iter().zip(b.iter())) {
                    *o = match op { BinOp::Add => x.wrapping_add(y) & m,
        ...
        Self { bits: self.bits, modulus: self.modulus, mask: self.mask, storage }
    }

    /// Element-wise addition / subtraction / multiplication
    pub fn add(&self, other: &Self) -> Self { self.bin_op(other, BinOp::Add) }
    pub fn sub(&self, other: &Self) -> Self { self.bin_op(other, BinOp::Sub) }
    pub fn mul(&self, other: &Self) -> Self { self.bin_op(other, BinOp::Mul) }

    /// Compress to bit-packed bytes (same strategy as original RingVec)
    pub fn to_bytes(&self) -> Vec<u8> {
        let bits_per_element = self.bits as usize;
        if bits_per_element == 0 { return Vec::new(); }
        let len = self.len();
        ...
        for i in 0..len {
            ...
            for bit_pos in 0..bits_per_element {
                if (val >> bit_pos) & 1 == 1 {
                    ...
                    let byte_idx = global / 8; let bit_in = global % 8;
    ...
    pub fn from_bytes(bytes: &[u8], len: usize, modulus: u128) -> Result<(Self, usize), String> {
        let bits = Self::modulus_to_bits(modulus);
        let total_bits = len * bits; let padded_bits = (total_bits + 7) & !7; let bytes_needed = padded_bits / 8;
        if bytes.len() < bytes_needed { return Err(format!("not enough bytes: need {}, have {}", bytes_needed, bytes.len())); }
        let mut v = Self::zero(len, modulus);
        for i in 0..len {
            let bit_start = i * bits; let mut value = 0u128;
            for bit_pos in 0..bits { let global = bit_start + bit_pos; let byte_idx = global / 8; let bit_in = global % 8; if (bytes[byte_idx] >> bit_in) & 1 == 1 { value |= 1u128 << bit_pos; } }
            v.set(i, value);
        }
        Ok((v, bytes_needed))
    }
...
mod tests {
    ...
    fn test_roundtrip_bytes() {
        let bits = 14usize; let modulus = 1u128 << bits; let len = 25; let v = DynRingVec::random(len, modulus);
        let bytes = v.to_bytes(); let (dec, used) = DynRingVec::from_bytes(&bytes, len, modulus).unwrap(); assert_eq!(used, bytes.len());
        for i in 0..len { assert_eq!(v.get(i), dec.get(i)); }
    }
}
