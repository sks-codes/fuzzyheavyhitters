use counttree::fss::right_interval::RIntervalFSSKey;
use counttree::data_structures::payload::RingVec;
use counttree::{u32_to_bits, bits_to_u32};

#[test]
fn test_rintervalfss_basic_functionality() {
    const N: usize = 1;
    let nbits = 5;
    ...
    // Generate FSS keys
    let (key0, key1) = RIntervalFSSKey::gen_RIntervalFSSKey(
        &alpha_bits, 
        &beta_bits, 
        payload_left, 
        payload_mid, 
    ...
    for prefix_length in 1..(nbits+1) {
        for x in 0u32..((1<<prefix_length) as u32) {
            ...
            let eval0 = key0.eval_rintervalFSS(&x_bits, modulus);
            let eval1 = key1.eval_rintervalFSS(&x_bits, modulus);
            println!();
